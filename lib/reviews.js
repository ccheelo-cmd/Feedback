...
// Break a name into comparable tokens (lowercased, punctuation stripped,
// single-character tokens dropped so initials don't cause false matches).
...
// Two names refer to the same person if they're equal once normalized, or if
// they share any name token. First names are unique within the cohort, so
// entering just a first (or last) name still resolves to the same person.
...
// Prefer the more complete name (more tokens) so re-entering just a first name
// doesn't shrink a previously stored full name.
function pickFullerName(existing, incoming) {
  return nameTokens(existing).length > nameTokens(incoming).length ? existing : incoming;
}

// Find the existing row for a given name. Returns the raw row
// { id, review, created_at } or null. Matching is done in JS so we don't depend
// on jsonb filter syntax — fine for a single cohort's worth of responses.
async function findRowByName(name) {
  ...
  return data.find((row) => namesMatch(row.review?.name, name)) || null;
}

// Upsert by name: if someone already submitted under this name, update that
// row (an edit); otherwise insert a new one. submittedAt is (re)stamped here so
// it always reflects the latest save. Returns { updated }.
export async function saveReview(review) {
  if (!supabase) throw new Error('Supabase is not configured.');
  const existing = await findRowByName(review.name);
  const stamped = { ...review, submittedAt: new Date().toISOString() };
  if (existing) {
    stamped.name = pickFullerName(existing.review?.name, review.name);
    const { error } = await supabase.from('reviews').update({ review: stamped }).eq('id', existing.id);
    if (error) throw error;
    return { updated: true };
  }
  const { error } = await supabase.from('reviews').insert({ review: stamped });
  if (error) throw error;
...
export async function getReviewByName(name) {
  ...
  return row ? { ...row.review, submittedAt: row.review?.submittedAt || row.created_at } : null;
}
...
export async function loadResponses() {
  ...
  const { data, error } = await supabase
    ...
    .order('created_at', { ascending: false });
  if (error) throw error;
  // submittedAt is stamped server-side on every save, so it reflects the last
  // update; fall back to created_at for any legacy rows without it.
  return data.map((item) => ({
    ...item.review,
    submittedAt: item.review?.submittedAt || item.created_at,
  }));
}
...
export function renderResponsesPage(responses) {
  ...
  const respondents =
    rows
      .map(
        (r) =>
          `<li><span>${escapeHtml(r.name || '—')}</span><b>${escapeHtml(r.email || 'no email')}</b><time>updated ${escapeHtml(new Date(r.submittedAt).toLocaleString())}</time></li>`
      )
